              ...
              <table id="tbl_id" class="table table-bordered table-striped">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                        <div class="btn-group">
                          <a href="<?= base_url('admin/invoice_generate/').$val->r_id.'/'.$val->r_user_id; ?>"><button type="button" class="btn btn-primary btn-sm"  data-id="<?php echo  $val->r_id; ?>">Billing</button></a>

                      <a href="<?= base_url('admin/update_reject_request/').$val->r_id.'/'.$val->r_user_id.'/'.$val->r_status; ?>"><button  class="<?= ($val->r_status == 'rejected')?'btn btn-secondary':'btn btn-danger' ?> btn-sm" data-id="<?php echo $val->r_id; ?>"><?= ($val->r_status == 'rejected')?'rejected':'Reject' ?></button></a>

                    <a href="#" data-toggle="modal" data-target="#modalview-lg<?= $val->r_id ?>"><button type="button" class="btn btn-info btn-sm">View</button></a>

                    <a href="#" data-toggle="modal" data-target="#modal-sm<?= $val->r_id ?>">
                      <button type="button" class="btn btn-warning btn-sm"><i class="fa fa-trash text-danger" title="Delete"></button></i>
                    </a>
      ...
      <div class="modal fade" id="modal-sm<?= $val->r_id?>">
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            ...
            <form action="<?php echo base_url();?>admin/delete-page/requests" method="post"> 
            <div class="modal-body">
...
<script type="text/javascript">
   ...
   //    let id = $('#reject_request').data('id');
      // alert(id);
      // $.ajax({
      //   url:url+"admin/update_reject_request/"+id,
      //   type:'GET',
      //   success: (result) => {
      //      if(result){
      ...
      // });
        // $.ajax({
        //       url: "<?php //echo base_url("admin/update_reject_request/"); ?>" + id,
        //       type: "GET",
        //       cache: false,
        ...
        //       }
        //     });
 //   ajax(url+"admin/update_reject_request/"+id).then(function(result) {
 //     if(result){
 //       $('#reject_request').attr('disabled','disabled');
 //           setTimeout(()=>{
  ...
  alert(url)  
   ...
   $('#invoice_generate').on('click' , function (e){
    ...
    ajax(url+"admin/invoice_generate/"+id).then(function(result) {
      if(result){
        $('#messageForm').html('<div class="alert alert-success">Successfully Invoice Generate</div>');
        $(this).attr('disabled','disabled');
      }
      else {
        $('#messageForm').html('<div class="alert alert-danger">Failed Invoice Generate</div>');
      }
    }).catch(function(e) {
      console.log(e)
    })
