...
class Login_model extends CI_Model{
	...
	|-------------------------------------------------------------------------*/
	public function sign_up($company_name,$username, $email,$vendor,$password,$country,$mobile) {
		$this->db->set('user_company_name',$company_name);
		$this->db->set('user_name',$username);
		$this->db->set('user_password',$password);
		$this->db->set('user_email',$email);
		$this->db->set('user_vendor_type',$vendor);
		$this->db->set('user_country',$country);
	...
	public function sign_in($email,$password) {
		$checkUser = $this->db->get_where('users',['user_email'=>$email]);
		if($checkUser->row('user_password') === $password){
	...
	public function checkEmailExists($email) {
		$email = $this->db->get_where('users',['user_email'=>$email]);
		if($email->num_rows() === 1){
			return true;
		} else {
			return false;
    ...
    /*-------------------------------------------------------------------------
    | Method : changePassword($new_password);
    |-------------------------------------------------------------------------*/
    public function changePasswordVendor($new_password,$email){
    	$this->db->where('user_email',$email);
    	$query = $this->db->update('users',['user_password'=>sha1($new_password)]);
    	if($query){
    		return true;
    	} else {
    		return false;
